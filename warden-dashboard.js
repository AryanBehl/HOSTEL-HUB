document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadComplaints() {
        try {
            ...
            document.getElementById('complaintsList').innerHTML = '<div style="padding:20px;text-align:center;color:red;">Cannot connect to server</div>';
        }
    }
    ...
    function displayComplaints(complaints) {
        ...
        document.querySelectorAll('.resolve-btn').forEach(function(btn) {
            btn.addEventListener('click', async function() {
                ...
                try {
                    const res = await fetch(`${BACKEND_URL}/api/complaint/resolve/${id}`, { method: 'PUT' });
                    const data = await res.json();
                    if (data.success) {
    ...
    function updateStats(complaints) {
        ...
        const pendingList = complaints.filter(function(c) { return c.status === 'pending'; }).slice(0, 5);
        if (pendingList.length === 0) {
            recentContainer.innerHTML = '<div style="padding:20px;text-align:center;">No pending complaints</div>';
        } else {
            let html = '';
            ...
            document.querySelectorAll('.resolve-recent').forEach(function(btn) {
                btn.addEventListener('click', async function() {
                    const id = this.getAttribute('data-id');
                    if (!confirm('Resolve?')) return;
                    await fetch(`${BACKEND_URL}/api/complaint/resolve/${id}`, { method: 'PUT' });
                    loadComplaints();
                });
            });
        }
    ...
    document.querySelectorAll('.filter-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            rows.forEach(function(row) {
                ...
                else if (filter === 'pending' && row.querySelector('span') && row.querySelector('span').innerText === 'Pending') row.style.display = 'grid';
                else if (filter === 'resolved' && row.querySelector('span') && row.querySelector('span').innerText === 'Resolved') row.style.display = 'grid';
